    ...
    return (
        <Modal opened={opened} onClose={handleCerrar} centered size="md"
            ...
            <Stack gap="md" align="center">
                <Box style={{width: 300, height: 300, background: "#f1f3f5", borderRadius: 12, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center"}}>
                    {preview || fotoUrl
...
function ModalSubcategorias({modal, subsList, nuevaSub, setNuevaSub, onAgregar, onEliminar, onClose, onEditarNombre, soloActivas, setSoloActivas}) {
            ...
            <Stack gap="sm">
                ...
                {lista.map(s => (
                    <Group key={s.id} justify="space-between" px="xs" py={4}
                        ...
                        <Group gap={4}>
                            <Badge size="xs" color={s.activa ? "green" : "gray"}>{s.activa ? "Activa" : "Oculta"}</Badge>
                            <ActionIcon color="blue" variant="light" size="sm" onClick={() => onEditarNombre(s)}>
                                <i className="fas fa-edit" style={{fontSize: 11}}/>
                            </ActionIcon>
                            <ActionIcon color={s.activa ? "orange" : "green"} variant="light" size="sm" onClick={() => onEliminar(s.id)}>
...
function TablaCategorias({records, searchCat, setSearchCat, onCrear, onEditar, onSubs, soloActivas, setSoloActivas}) {
    return (
        <div className="flex flex-1 flex-col border px-4 py-4 rounded-2xl">
            <CabeceraTabla titulo="Categorias" onCrear={onCrear} search={searchCat} setSearch={setSearchCat} placeholder="Buscar categoria...">
                <Switch label="Solo activas" checked={soloActivas} onChange={e => setSoloActivas(e.currentTarget.checked)}/>
            </CabeceraTabla>
            <div className="h-[200px]">
                <DataTable noRecordsText="Sin resultados" withTableBorder withColumnBorders striped highlightOnHover idAccessor="id"
                           columns={[
                               {accessor: "nombre", title: "Nombre"},
                               {accessor: "hijos", title: "Subcategorias", render: ({hijos}) => <Badge color="blue" variant="light">{(hijos ?? []).length}</Badge>},
                               {accessor: "activa", title: "Estado", render: ({activa}) => <Badge color={activa ? "green" : "gray"}>{activa ? "Activa" : "Oculta"}</Badge>},
                           ]}
                           records={records}
                           rowExpansion={{
                               allowMultiple: false,
                               content: ({record}) => (
                                   <Box p="sm" bg="gray.0">
                                       <div className="flex justify-between items-center mb-3">
                                           <Text size="xs" fw={600} tt="uppercase" c="dimmed">Subcategorias de {record.nombre}</Text>
...
function TablaAtributos({records, searchAtr, setSearchAtr, onCrear, onEditar}) {
    return (
        <div className="flex flex-1 flex-col border px-4 py-4 rounded-2xl">
            <CabeceraTabla titulo="Atributos" onCrear={onCrear} search={searchAtr} setSearch={setSearchAtr} placeholder="Buscar atributo..."/>
            <div className="h-[200px]">
                <DataTable noRecordsText="Sin resultados" withTableBorder withColumnBorders striped highlightOnHover idAccessor="id"
                           columns={[
                               {accessor: "nombre", title: "Nombre", width: 300},
                               {accessor: "acciones", title: "", render: (a) => <Button color="dark" size="xs" onClick={() => onEditar(a)}><i className="fas fa-edit"/></Button>},
                           ]}
                           records={records}
                />
            </div>
                    ...
                    <Select label="Categoria" placeholder="Seleccionar..." searchable data={opsCat}
                            ...
                            renderOption={({option}) => (
                                <Group gap="xs">
                                    ...
                                    {option.inactiva && <Badge size="xs" color="red" variant="light">(inactivo)</Badge>}
                                </Group>
                            )}/>
                    <Select label="Unidades" data={opsUnd} value={form.unidades} onChange={val => setForm({...form, unidades: val})}/>
                </Group>
                <Textarea label="Descripcion" placeholder="Descripcion del producto..." rows={3}
                          value={form.descripcion} onChange={e => setForm({...form, descripcion: e.currentTarget.value})}/>
                <Switch label="Publicado" checked={form.publicado} onChange={e => setForm({...form, publicado: e.currentTarget.checked})}/>
                <BotonesModal onClose={onClose} onGuardar={onGuardar} saving={saving} esEdicion={!!modal.data}/>
            </Stack>
        </Modal>
    );
...
function TablaProductos({records, tiposAtributo, searchProd, setSearchProd, onCrear, onEditar, onCrearVar, onEditarVar, onModificarStock, onVerFoto, onVerFotoVariante, onExpandir, soloActivos, setSoloActivos, soloActivosVar, setSoloActivosVar, variantesCache}) {
    return (
        <div className="flex flex-col border px-4 py-4 rounded-2xl">
            <CabeceraTabla titulo="Productos" onCrear={onCrear} search={searchProd} setSearch={setSearchProd} placeholder="Buscar producto...">
                <Switch label="Solo publicados" checked={soloActivos} onChange={e => setSoloActivos(e.currentTarget.checked)}/>
            </CabeceraTabla>
                           ...
                           columns={[
                               ...
                               {accessor: "marca", title: "Marca"},
                               {
                                   ...
                                   render: ({categoria, categoriaActiva}) => (
                                       <Group gap={4}>
                                           <Badge variant="light" color={categoriaActiva === false ? "red" : "blue"}>{categoria}</Badge>
                                           {categoriaActiva === false && <Badge size="xs" color="red" variant="light">(inactivo)</Badge>}
                                       </Group>
                                   )
                               },
                                   ...
                                   render: ({variantes}) => {
                                       ...
                                       return (
                                           <Group gap={4}>
                                               ...
                                               {inactivas > 0 && <Badge color="yellow">{inactivas} inactiva{inactivas > 1 ? "s" : ""}</Badge>}
                                               {sinStock > 0 && <Badge color="red">{sinStock} sin stock</Badge>}
                                           </Group>
                                       );
                                   }
                               },
                               {accessor: "publicado", title: "Publicado", render: ({publicado}) => <Badge color={publicado ? "green" : "gray"}>{publicado ? "Publicado" : "Borrador"}</Badge>},
                           ]}
                           records={records}
                           rowExpansion={{
                               allowMultiple: false,
                                       ...
                                       <Box p="sm" bg="gray.0">
                                           <div className="flex justify-between items-center mb-3">
                                               <div className="flex flex-col gap-1">
                                                   <Text size="xs" fw={600} tt="uppercase" c="dimmed">Variantes de {record.nombre}</Text>
                                                   <Text size="xs" c="dimmed">{record.descripcion}</Text>
                                               </div>
                                               <Switch label="Solo activas" checked={soloActivosVar} onChange={e => setSoloActivosVar(e.currentTarget.checked)}/>
                                               <div className="flex gap-2">
                                                   <Button color="violet" size="xs" onClick={() => onVerFoto(record)}>
                                           ...
                                           <DataTable key={`${record.id}-${variantesFiltradas.length}`} withTableBorder withColumnBorders idAccessor="id" fontSize="xs"
                                                      columns={[
                                                                  ...
                                                                  <Group gap={4}>
                                                                      {(atributos ?? []).length === 0
                                                                          ...
                                                                          : atributos.map((a, i) => {
                                                                              ...
                                                                              return <Badge key={i} variant="light" color="blue" size="sm">{tipo?.nombre ?? "?"}: {a.valor}</Badge>;
                                                                          })}
                                                                  </Group>
                                                              )
                                                          },
                                                          {accessor: "precio_venta", title: "Venta", render: ({precio_venta}) => "S/ " + Number(precio_venta).toFixed(2)},
                                                          {accessor: "precio_mayorista", title: "Mayorista", render: ({precio_mayorista}) => "S/ " + Number(precio_mayorista).toFixed(2)},
                                                          {accessor: "precio_minimo", title: "Minimo", render: ({precio_minimo}) => "S/ " + Number(precio_minimo).toFixed(2)},
                                                          {
                                                          ...
                                                          {accessor: "activa", title: "Estado", render: ({activa}) => <Badge color={activa ? "green" : "gray"}>{activa ? "Activa" : "Inactiva"}</Badge>},
                                                          {
                                                              accessor: "acciones", title: "",
                                                              render: (variante) => (
                                                                  <Group gap={4}>
                                                                      <Button color="violet" size="xs" onClick={() => onVerFotoVariante(variante)}>
                                                                          <i className="fas fa-image"/>
                                                                      </Button>
                                                                      <Button color="dark" size="xs" onClick={() => onEditarVar(variante, record)}>
                                                                          <i className="fas fa-edit"/>
                                                                      </Button>
                                                                      <Button color="teal" size="xs" onClick={() => onModificarStock(variante)}>
            ...
            <Stack gap="sm">
                ...
                <Group grow>
                    ...
                    <NumberInput label="Precio mayorista (S/)" step={10} decimalScale={2} min={0}
                                 value={form.precio_mayorista} onChange={val => setForm(f => ({...f, precio_mayorista: val}))}/>
                    <NumberInput label="Precio minimo (S/)" step={10} decimalScale={2} min={0}
                                 value={form.precio_minimo} onChange={val => setForm(f => ({...f, precio_minimo: val}))}/>
                </Group>
                            ...
                            : form.atributos.map((a, i) => (
                                <Group key={i} gap="xs">
                                    <Select placeholder="Tipo de atributo" searchable style={{flex: 1}}
                                            data={opsTipoAtr} value={a.tipo} onChange={val => actualizarAtr(i, "tipo", val)}/>
                                    <TextInput size="xs" placeholder="Valor (ej. 15mm)" value={a.valor} style={{flex: 1}}
...
function ModalStock({modal, onClose, onGuardar, saving}) {
    ...
    return (
        <Modal opened={modal.open} onClose={onClose} centered size="sm"
               title={<Text fw={700}>Modificar Stock — Actual: <Text span c="teal" fw={800}>{stockActual}</Text></Text>}>
            <Stack gap="md">
                ...
                <BotonesModal onClose={onClose} onGuardar={() => !invalido && onGuardar(operacion, cantidad)} saving={saving} esEdicion={true}/>
            </Stack>
        </Modal>
    );
}
        ...
        <Modal opened={opened} onClose={onClose} centered size="xs"
            ...
            <Stack gap="md">
                ...
                <TextInput label="Código de verificación" placeholder="000000" maxLength={6} value={codigo}
                           ...
                           error={error} onKeyDown={e => e.key === "Enter" && codigo.length === 6 && onConfirmar()} autoFocus/>
                <Group justify="flex-end">
                    <Button variant="default" onClick={onClose}>Cancelar</Button>
                    <Button color="teal" loading={loading} disabled={codigo.length !== 6} onClick={onConfirmar}>Confirmar</Button>
                </Group>
            </Stack>
        </Modal>
    );
...
export default function Productos() {
    ...
    const guardarFoto = async (archivo) => {
        ...
        try {
            ...
            setModalFoto(prev => ({...prev, url: (modalFoto.tipo === "producto" ? api.productos.obtener_foto(modalFoto.id) : api.productos.obtener_foto_variante(modalFoto.id)) + "?t=" + Date.now()}));
        } finally {
            setSavingFoto(false);
        }
    };
    ...
    const abrirSubs = (c) => { setSubsList([...(c.hijos ?? [])]); setNuevaSub(""); setModalSub({open: true, data: c}); };
    ...
    const guardarVar = () => vari.guardar(async () => {
        ...
        if (vari.modal.data) {
            ...
            for (const a of atributos)
                if (a.tipo && a.valor) await api.productos.agregar_atributo({variante_id: vari.modal.data.id, tipo_atributo_id: a.tipo, valor: a.valor});
        } else {
            const nueva = await api.productos.agregar_variante({...datosVariante, producto_base_id: vari.modal.producto.id});
            for (const a of atributos)
                if (a.tipo && a.valor) await api.productos.agregar_atributo({variante_id: nueva.id, tipo_atributo_id: a.tipo, valor: a.valor});
        }
        await cargarTodo();
    });
    ...
    const guardarStock = async (operacion, cantidad) => {
        ...
        if (codigoStock) { ejecutarCambioStock(operacion, cantidad, codigoStock); return; }
        try {
            await api.productos.solicitar_codigo_stock();
            setCodigoVerif(""); setVerifError(""); setModalVerif(true);
        } catch (e) {
    ...
    const ejecutarCambioStock = async (operacion, cantidad, codigo) => {
        ...
        try {
            ...
            stock.cerrar(); await cargarTodo(); noti_util("success", "Stock actualizado correctamente");
        } catch (e) {
            const msg = e?.message ?? "Código incorrecto";
            if (msg.includes("expirado")) {
                setCodigoStock(null); noti_util("error", "El código expiró, se enviará uno nuevo"); guardarStock(operacion, cantidad);
            } else {
                setCodigoStock(null); setCodigoVerif(""); setVerifError(msg); setModalVerif(true);
            }
        }
    };

    const confirmarVerificacion = async () => {
        setVerifLoading(true); setVerifError("");
        try {
            const {operacion, cantidad} = pendienteStock;
            await ejecutarCambioStock(operacion, cantidad, codigoVerif);
            setCodigoStock(codigoVerif); setModalVerif(false);
        } catch (e) {
    ...
    const opsCat = categorias.flatMap(c => {
        const hijos = c.hijos ?? [];
        if (hijos.length > 0) return hijos.map(h => ({value: String(h.id), label: `${c.nombre} > ${h.nombre}${h.activa ? "" : " (inactivo)"}`, inactiva: !h.activa}));
        return [{value: String(c.id), label: c.nombre, inactiva: false}];
    });
    const opsTipoAtr = tiposAtributo.map(a => ({value: a.id, label: a.nombre}));
    const recordsCat = categorias.filter(c => c.nombre.toLowerCase().includes(searchCat.toLowerCase())).filter(c => !soloActivasCat || c.activa).map(c => ({...c, hijos: soloActivasCat ? (c.hijos ?? []).filter(h => h.activa) : (c.hijos ?? [])}));
    ...
    return (
        <div className="flex flex-col gap-6" style={{position: "relative"}}>
            <ModalCategoria modal={cat.modal} form={cat.form} setForm={cat.setForm} saving={cat.saving} onClose={cat.cerrar} onGuardar={guardarCat}/>
            <ModalSubcategorias modal={modalSub} subsList={subsList} nuevaSub={nuevaSub} setNuevaSub={setNuevaSub}
                                ...
                                onEditarNombre={editarNombreSub} soloActivas={soloActivasSub} setSoloActivas={setSoloActivasSub}/>
            <ModalAtributo modal={atr.modal} form={atr.form} setForm={atr.setForm} saving={atr.saving} onClose={atr.cerrar} onGuardar={guardarAtr}/>
            <ModalProducto modal={prod.modal} form={prod.form} setForm={prod.setForm} saving={prod.saving} onClose={prod.cerrar} onGuardar={guardarProd} opsCat={opsCat}/>
            <ModalVariante modal={vari.modal} form={vari.form} setForm={vari.setForm} saving={vari.saving} onClose={vari.cerrar} onGuardar={guardarVar} opsTipoAtr={opsTipoAtr}/>
            <ModalStock modal={stock.modal} onClose={stock.cerrar} onGuardar={(op, cant) => guardarStock(op, cant)}/>
            <ModalVerificacion opened={modalVerif} onClose={() => setModalVerif(false)} onConfirmar={confirmarVerificacion}
                               loading={verifLoading} error={verifError} codigo={codigoVerif} setCodigo={setCodigoVerif}/>
            <ModalFoto opened={modalFoto.open} onClose={() => setModalFoto(prev => ({...prev, open: false}))}
                       fotoUrl={modalFoto.url} onCambiarFoto={guardarFoto} saving={savingFoto} titulo={modalFoto.titulo}/>
            <div className="flex flex-1 justify-between gap-6">
                <TablaCategorias records={recordsCat} searchCat={searchCat} setSearchCat={setSearchCat}
                                 ...
                                 onSubs={abrirSubs} soloActivas={soloActivasCat} setSoloActivas={setSoloActivasCat}/>
            ...
            <TablaProductos
                records={recordsProd} tiposAtributo={tiposAtributo} searchProd={searchProd} setSearchProd={setSearchProd}
                onCrear={() => prod.abrir(null, emptyProd)}
                onEditar={p => prod.abrir(p, {nombre: p.nombre, marca: p.marca, categoria_id: p.categoria_id ? String(p.categoria_id) : null, unidades: p.unidades, descripcion: p.descripcion, publicado: p.publicado})}
                onCrearVar={p => vari.abrir({producto: p}, emptyVar)}
                onEditarVar={(v, p) => {
                    vari.abrir(v, {sku: v.sku, precio_venta: v.precio_venta, precio_mayorista: v.precio_mayorista, precio_minimo: v.precio_minimo, activa: v.activa, atributos: (v.atributos ?? []).map(a => ({...a}))});
                    vari.modal.producto = p;
                }}
