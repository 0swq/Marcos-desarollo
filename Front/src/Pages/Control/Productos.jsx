                               ...
                               content: ({record}) => {
                                   ...
                                   return (
                                       <Box p="sm" bg="gray.0">
                                           ...
                                           <DataTable
                                               key={`${record.id}-${variantesFiltradas.length}`}
...
function ModalVerificacion({opened, onClose, onConfirmar, loading, error, codigo, setCodigo, tieneTOTP}) {
    return (
            ...
            <Stack gap="md">
                {tieneTOTP ? (
                    <>
                        <Text size="sm" c="dimmed">
                            Ingresa el código de 6 dígitos de tu aplicación autenticadora (Google Authenticator).
                        </Text>
                        <TextInput
                    ...
                    <>
                        ...
                        <TextInput
                            ...
                            error={error}
                            onKeyDown={e => e.key === "Enter" && codigo.length > 0 && onConfirmar()}
                            autoFocus
                        />
                    </>
                )}
                <Group justify="flex-end">
                    ...
                    <Button color="teal" loading={loading}
                            disabled={tieneTOTP ? codigo.length !== 6 : codigo.length === 0}
                            onClick={onConfirmar}>
                        Confirmar
                    </Button>
                </Group>
...
export default function Productos() {
    ...
    // Detecta si el usuario tiene TOTP configurado
    const tieneTOTP = user?.twoFactorEnabled &&
    ...
    const guardarStock = (operacion, cantidad) => {
    ...
    try {
        if (tieneTOTP) {
            ...
            await user.verifyPassword(codigoVerif);
        }
        ...
        const response = await fetch(`${BASE_URL}/producto/variantes/${stock.modal.data.id}/stock`, {
            method: "PATCH",
        ...
        const msg = e?.errors?.[0]?.message ?? e?.message ?? "Código incorrecto";
        setVerifError(msg);
    } finally {
