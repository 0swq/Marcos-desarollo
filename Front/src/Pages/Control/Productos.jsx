import {
    ActionIcon, Badge, Box, Button, Group, Modal, NumberInput,
...
function useFormErrors() {
    ...
    const validar = (reglas) => {
        const nuevosErrores = {};
        for (const [campo, {valor, mensaje}] of Object.entries(reglas)) {
            const vacio =
                valor === null ||
                valor === undefined ||
                (typeof valor === "string" && !valor.trim()) ||
                valor === "";
            if (vacio) nuevosErrores[campo] = mensaje;
        }
        setErrors(nuevosErrores);
        return Object.keys(nuevosErrores).length === 0;
    };

    const limpiar = () => setErrors({});

    return {errors, validar, limpiar};
}
    ...
    return (
        <Modal opened={opened} onClose={handleCerrar} centered size="md"
            ...
            <Stack gap="md" align="center">
                <Box style={{
                    width: 300, height: 300, background: "#f1f3f5", borderRadius: 12,
...
function ModalCategoria({modal, form, setForm, saving, onClose, onGuardar, errors = {}}) {
    return (
        <Modal opened={modal.open} onClose={onClose} centered size="sm"
            ...
            <Stack gap="sm">
                <TextInput
                    label="Nombre" placeholder="Ej. Tableros" required
                    value={form.nombre}
                    error={errors.nombre}
...
function ModalSubcategorias({
    modal, subsList, nuevaSub, setNuevaSub, onAgregar, onEliminar,
    onClose, onEditarNombre, soloActivas, setSoloActivas
}) {
            ...
            <div className="h-[200px]">
                <DataTable noRecordsText="Sin resultados" withTableBorder withColumnBorders striped highlightOnHover
                           ...
                           columns={[
                               ...
                               {
                                   accessor: "hijos", title: "Subcategorias",
                                   render: ({hijos}) => <Badge color="blue" variant="light">{(hijos ?? []).length}</Badge>
                               },
                               {
                                   accessor: "activa", title: "Estado",
                                   render: ({activa}) => <Badge color={activa ? "green" : "gray"}>{activa ? "Activa" : "Oculta"}</Badge>
                               },
                           ]}
                           records={records}
                           rowExpansion={{
...
function ModalAtributo({modal, form, setForm, saving, onClose, onGuardar, errors = {}}) {
    return (
        <Modal opened={modal.open} onClose={onClose} centered size="sm"
               title={<Text fw={700}>{modal.data ? "Editar Atributo" : "Nuevo Atributo"}</Text>}>
            <Stack gap="sm">
                <TextInput
                    label="Nombre del atributo" placeholder="Ej. Grosor, Color" required
                    value={form.nombre}
                    error={errors.nombre}
                    onChange={e => setForm({...form, nombre: e.currentTarget.value})}
                />
...
function ModalProducto({modal, form, setForm, saving, opsCat, onClose, onGuardar, errors = {}}) {
    return (
        <Modal opened={modal.open} onClose={onClose} centered size="lg"
            ...
            <Stack gap="sm">
                <Group grow>
                    <TextInput
                        label="Nombre" placeholder="Ej. Tablero MDF" required
                        value={form.nombre}
                        error={errors.nombre}
                        onChange={e => setForm({...form, nombre: e.currentTarget.value})}
                    />
                    <Select
                        ...
                        onCreate={query => {
                            setForm(f => ({...f, marca: query}));
                            return query;
                        }}
                        data={[...new Set([
                            "PEGASO", "TRUPER", "MG", "BOSCH", "C&A", "PRETUL",
                            "REPRESENTACIONES", "INGCO", "ALITORN", "HALCON", "VENEZIA",
                            "HOME LIGHT", "FERRACOM", "GRUPO KAPUSI S.A.", "VENEZZIA",
                            "HERMEX", "3M", "PHELIX", "FABRICLAVOS", "HOME CENTER",
                            "TIAN YU", "GY", "DUCASSE", "BROCE", "3 EN 1", "MAKITA",
                            "C&A PRO", "KLINGSPOR", "MADERA GILBERT", "ARVALER", "DIALED",
                            "DURATEX", "INVERSIONES JO", "MADERAS AMERICA", "YOFASA",
                            "ARAUCO", "TABLEROS HISPANOS", "NOVOPAN", "TY (PR) MANUFACTURING",
                            "MASISA", "PG BISON", "FERRETERIA MELESA", "GOLDENPLAST",
                            "KNAUFF", "AGLOME PERÚ S.A.", "FORTTE", "CISER",
                            "Duraplac", "Eucatex", "Egger", "Finsa", "Sonae", "Swiss Krono",
                            "Pfleiderer", "Uniboard", "Flakeboard", "Norbord", "Weyerhaeuser", "Kronospan",
                ...
                <Group grow>
                    <Select
                        label="Categoria" placeholder="Seleccionar..." searchable
                        data={opsCat}
...
function TablaProductos({
    records, tiposAtributo, searchProd, setSearchProd, onCrear, onEditar,
    onCrearVar, onEditarVar, onModificarStock, onVerFoto, onVerFotoVariante,
    onExpandir, soloActivos, setSoloActivos, soloActivosVar, setSoloActivosVar, variantesCache
}) {
    return (
                ...
                <DataTable noRecordsText="Sin resultados" withTableBorder withColumnBorders striped highlightOnHover
                           ...
                           columns={[
                               ...
                               },
                               {
                                   ...
                                   render: ({publicado}) => (
                                       <Badge color={publicado ? "green" : "gray"}>{publicado ? "Publicado" : "Borrador"}</Badge>
                                   )
                               },
                           ]}
                           records={records}
                                   ...
                                   return (
                                       <Box p="sm" bg="gray.0">
                                           ...
                                           <DataTable key={`${record.id}-${variantesFiltradas.length}`}
                                                      withTableBorder withColumnBorders idAccessor="id" fontSize="xs"
                                                      columns={[
...
function ModalVariante({modal, form, setForm, saving, opsTipoAtr, onClose, onGuardar, errors = {}}) {
    ...
    return (
        <Modal opened={modal.open} onClose={onClose} centered size="lg"
               title={
                   <Text fw={700}>
                       {modal.data ? "Editar Variante" : "Nueva Variante"}
                       {modal.producto && <Text span c="dimmed" fw={400}> — {modal.producto.nombre}</Text>}
                   </Text>
               }>
            <Stack gap="sm">
                <TextInput
                    label="SKU" placeholder="Ej. MDF-15-120" required
                    value={form.sku}
                    error={errors.sku}
                ...
                <Group grow>
                    <NumberInput
                        label="Precio venta (S/)" step={10} decimalScale={2} min={0} required
                        value={form.precio_venta}
                        error={errors.precio_venta}
                        onChange={val => setForm(f => ({...f, precio_venta: val}))}
                    />
                    <NumberInput
                        label="Precio mayorista (S/)" step={10} decimalScale={2} min={0}
                        value={form.precio_mayorista}
                        onChange={val => setForm(f => ({...f, precio_mayorista: val}))}
                    />
                    <NumberInput
                        label="Precio minimo (S/)" step={10} decimalScale={2} min={0}
...
function ModalStock({modal, onClose, onGuardar, saving}) {
    ...
    const nuevoStock = operacion === "aumentar"
        ? stockActual + (cantidad || 0)
        : Math.max(0, stockActual - (cantidad || 0));
    ...
    return (
        <Modal opened={modal.open} onClose={onClose} centered size="sm"
               title={
                   <Text fw={700}>
                       Modificar Stock — Actual: <Text span c="teal" fw={800}>{stockActual}</Text>
                   </Text>
               }>
            <Stack gap="md">
        ...
        <Modal opened={opened} onClose={onClose} centered size="xs"
            ...
            <Stack gap="md">
                ...
                <TextInput label="Código de verificación" placeholder="000000" maxLength={6} value={codigo}
                           ...
                           error={error}
                           onKeyDown={e => e.key === "Enter" && codigo.length === 6 && onConfirmar()}
                           autoFocus/>
...
export default function Productos() {
    ...
    const erroresVar  = useFormErrors();
    ...
    const guardarFoto = async (archivo) => {
        ...
        try {
            ...
            setModalFoto(prev => ({
                ...prev,
                url: (modalFoto.tipo === "producto"
                    ? api.productos.obtener_foto(modalFoto.id)
                    : api.productos.obtener_foto_variante(modalFoto.id)) + "?t=" + Date.now()
            }));
        } finally {
            setSavingFoto(false);
        }
    ...
    const guardarProd = () => {
        if (!erroresProd.validar({
            ...
            categoria_id: {valor: prod.form.categoria_id, mensaje: "Selecciona una categoría"},
        })) return;
        erroresProd.limpiar();
        prod.guardar(async () => {
    ...
    const guardarVar = () => {
        if (!erroresVar.validar({
            ...
            precio_venta: {
                valor: vari.form.precio_venta !== "" && vari.form.precio_venta !== null && vari.form.precio_venta !== undefined
                    ? String(vari.form.precio_venta)
                    : "",
    ...
    return (
        <div className="flex flex-col gap-6" style={{position: "relative"}}>
            {/* ── Modales ── */}
            <ModalCategoria
                modal={cat.modal} form={cat.form} setForm={cat.setForm}
                saving={cat.saving} errors={erroresCat.errors}
                onClose={() => { cat.cerrar(); erroresCat.limpiar(); }}
                onGuardar={guardarCat}
            />
            <ModalSubcategorias
                modal={modalSub} subsList={subsList} nuevaSub={nuevaSub} setNuevaSub={setNuevaSub}
            ...
            />
            <ModalAtributo
                modal={atr.modal} form={atr.form} setForm={atr.setForm}
                saving={atr.saving} errors={erroresAtr.errors}
                onClose={() => { atr.cerrar(); erroresAtr.limpiar(); }}
                onGuardar={guardarAtr}
            />
            <ModalProducto
                modal={prod.modal} form={prod.form} setForm={prod.setForm}
                saving={prod.saving} errors={erroresProd.errors} opsCat={opsCat}
                onClose={() => { prod.cerrar(); erroresProd.limpiar(); }}
                onGuardar={guardarProd}
            />
            <ModalVariante
                modal={vari.modal} form={vari.form} setForm={vari.setForm}
                saving={vari.saving} errors={erroresVar.errors} opsTipoAtr={opsTipoAtr}
                onClose={() => { vari.cerrar(); erroresVar.limpiar(); }}
                onGuardar={guardarVar}
            />
            <ModalStock
                modal={stock.modal} onClose={stock.cerrar}
                onGuardar={(op, cant) => guardarStock(op, cant)}
            />
            <ModalVerificacion
                opened={modalVerif} onClose={() => setModalVerif(false)}
                onConfirmar={confirmarVerificacion}
                loading={verifLoading} error={verifError}
                codigo={codigoVerif} setCodigo={setCodigoVerif}
            />
            <ModalFoto
                opened={modalFoto.open} onClose={() => setModalFoto(prev => ({...prev, open: false}))}
                fotoUrl={modalFoto.url} onCambiarFoto={guardarFoto}
                saving={savingFoto} titulo={modalFoto.titulo}
            />

            {/* ── Tablas ── */}
            <div className="flex flex-1 justify-between gap-6">
                <TablaCategorias
                    records={recordsCat} searchCat={searchCat} setSearchCat={setSearchCat}
                ...
                />
                <TablaAtributos
                    records={recordsAtr} searchAtr={searchAtr} setSearchAtr={setSearchAtr}
                    onCrear={() => atr.abrir(null, emptyAtr)}
                    onEditar={a => atr.abrir(a, {nombre: a.nombre})}
                />
            </div>
            <TablaProductos
                records={recordsProd} tiposAtributo={tiposAtributo}
                searchProd={searchProd} setSearchProd={setSearchProd}
                ...
                onEditar={p => prod.abrir(p, {
                    ...
                    unidades: p.unidades, descripcion: p.descripcion, publicado: p.publicado
                })}
                onCrearVar={p => vari.abrir(null, emptyVar, {producto: p})}
                onEditarVar={(v, p) => vari.abrir(
                    v,
                    {
                        sku: v.sku, precio_venta: v.precio_venta, precio_mayorista: v.precio_mayorista,
                        precio_minimo: v.precio_minimo, activa: v.activa,
